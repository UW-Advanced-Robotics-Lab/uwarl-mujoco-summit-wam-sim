...
class Link:
    ...
    def __init__(self, col_meshes, com_pos, dclass_col, inertia, mass, name, viz_meshes, dclass_viz = None):
        # Make a MujoCo Root
        self.mjcf_model = mjcf.RootElement(model=name)
        # Add a body element:
...
class Bhand:
    ...
    def __init__(self, name):
        ...
        # Create 'palm link' 
        self.palm_link = Link(col_meshes = self._col_mesh_name[0],
        ...
        for finger_index in range(3):
          ...
          if finger_index in range(2):
            ...
            self.finger_index_prox_link.append(Link(col_meshes = self._col_mesh_name[1],
            ...
            self.finger_index_prox_link_site[finger_index].add('joint',
                                                               name = self._bhand_name+'\\'+self._finger_name_short[0+finger_index]+'\\'+self._body_name[1],
                                                               range = self._joint_range[0], 
                                                               damping = self._joint_damping,
                                                               type = self._joint_type,
                                                               axis = self._joint_axis[0],
          ...
          self.finger_index_med_link.append(Link(col_meshes = self._col_mesh_name[2],
          ...
          # Add joint to the attachment frame
          self.finger_index_med_link_site[finger_index].add('joint',
                                                            name = self._bhand_name+'\\'+self._finger_name_short[0+finger_index]+'\\'+self._body_name[2],
                                                            range = self._joint_range[1], 
                                                            damping = self._joint_damping,
                                                            type = self._joint_type,
                                                            axis = self._joint_axis[1],
                                                            frictionloss = self._joint_frictionloss,
                                                            dclass = 'rand')
          ...
          self.finger_index_dist_link.append(Link(col_meshes = self._col_mesh_name[3],
          ...
          # Add joint to the attachment frame
          self.finger_index_med_link_site[finger_index].add('joint',
                                                            name = self._bhand_name+'\\'+self._finger_name_short[0+finger_index]+'\\'+self._body_name[3],
                                                            range = self._joint_range[2], 
                                                            damping = self._joint_damping,
                                                            type = self._joint_type,
                                                            axis = self._joint_axis[2],
                                                            frictionloss = self._joint_frictionloss,
                                                            dclass = 'rand')
...
class WAM:
    ...
    def __init__(self, name):
        ...
        self.base_site = self.base_link.add('site',
                                            ...
                                            dclass = self._site_name)
        # Create link 'base'
        self.base = Link(col_meshes = self._col_mesh_names[0],
                         com_pos = self._com_pos[0], 
                         dclass_col = 'wam\\col',
                         mass = self._mass[0],
                         inertia = self._inertial[0],
        ...
        self.shoulder_yaw_link = Link(col_meshes = self._col_mesh_names[1],
                                      com_pos = self._com_pos[1], 
                                      dclass_col = 'wam\\col',
                                      inertia = self._inertial[1],
                                      mass = self._mass[1],
        ...
        # Find attachment-frame
        frame_1 = self.shoulder_yaw_link_site.find('attachment_frame',self._wam_name+'\\'+self._body_name[2])
        # Add joint to the attachment frame
        frame_1.add('joint',
                    name = self._joint_name[0],
                    range = self._joint_range[0], 
                    damping = self._joint_damping[0],
        ...
        self.shoulder_pitch_link = Link(col_meshes = self._col_mesh_names[2],
                                        com_pos = self._com_pos[2], 
                                        dclass_col = 'wam\\col',
                                        inertia = self._inertial[2],
                                        mass = self._mass[2],
        ...
        # Add joint to the attachment frame
        self.shoulder_pitch_link_site.add('joint',
        ...
        self.upper_arm_link = Link(col_meshes = self._col_mesh_names[3],
                                   com_pos = self._com_pos[3], 
                                   dclass_col = 'wam\\col',
                                   inertia = self._inertial[3],
                                   mass = self._mass[3],
        ...
        # Add joint to the attachment frame
        self.upper_arm_link_site.add('joint',
                                          name = self._joint_name[2],
                                          range = self._joint_range[2], 
                                          damping = self._joint_damping[2],
                                          axis = self._joint_axis,
                                          type = self._joint_type,
                                          frictionloss = self._joint_frictionloss,
                                          dclass = 'rand')
        ...
        # Add joint to the attachment frame
        self.forearm_link_site.add('joint',
                                   name = self._joint_name[3],
                                   range = self._joint_range[3], 
                                   damping = self._joint_damping[3],
                                   axis = self._joint_axis,
                                   type = self._joint_type,
                                   frictionloss = self._joint_frictionloss,
                                   dclass = 'rand')
        ...
        self.wrist_yaw_link = Link(col_meshes = self._col_mesh_names[5],
        ...
        # Add joint to the attachment frame
        self.wrist_yaw_link_site.add('joint',
                                     name = self._joint_name[4],
                                     range = self._joint_range[4], 
                                     damping = self._joint_damping[4],
                                     axis = self._joint_axis,
                                     type = self._joint_type,
                                     frictionloss = self._joint_frictionloss,
                                     dclass = 'rand')
        ...
        self.wrist_pitch_link = Link(col_meshes = self._col_mesh_names[6],
        ...
        # Add joint to the attachment frame
        self.wrist_pitch_link_site.add('joint',
                                       name = self._joint_name[5],
                                       range = self._joint_range[5], 
                                       damping = self._joint_damping[5],
                                       axis = self._joint_axis,
                                       type = self._joint_type,
                                       frictionloss = self._joint_frictionloss,
                                       dclass = 'rand')
        ...
        self.wrist_palm_link = Link(col_meshes = self._col_mesh_names[7],
        ...
        # Add joint to the attachment frame
        self.wrist_palm_link_site.add('joint',
                                      name = self._joint_name[6],
                                      range = self._joint_range[6], 
                                      damping = self._joint_damping[6],
                                      axis = self._joint_axis,
                                      type = self._joint_type,
                                      frictionloss = self._joint_frictionloss,
                                      dclass = 'rand')
        ...
        self.wrist_torque_link = Link(dclass_col = 'wam\\col',
                                      com_pos = self._com_pos[8],
