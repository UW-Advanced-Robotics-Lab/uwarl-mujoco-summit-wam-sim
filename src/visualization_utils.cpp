...
void MujocoVisualizationUtils::profiler_update(void)
{
    ...
    figconstraint.linepnt[0] = mjMIN(mjMIN(*mujoco_data_->solver_niter, mjNSOLVER), mjMAXLINEPNT);
    for (i = 1; i < 5; i++)
    ...
    // update cost figure
    figcost.linepnt[0] = mjMIN(mjMIN(*mujoco_data_->solver_niter, mjNSOLVER), mjMAXLINEPNT);
    for (i = 1; i < 3; i++)
    ...
    float sdata[6] =
    {
        ...
        static_cast<float>((float) *mujoco_data_->solver_niter)
    };
